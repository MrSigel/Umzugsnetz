...
setTimeout(() => setCopyStatus('idle'), 2000);
  ...
  const handleShare = async (channel: 'facebook' | 'x' | 'whatsapp' | 'instagram' | 'email' | 'copy') => {
    ...
    const shareText = 'Vergleichen Sie kostenlos Angebote für Umzug und Entrümpelung auf Umzugsnetz.';
    ...
    if (channel === 'instagram') {
      await navigator.clipboard.writeText(shareUrl);
      showToast('info', 'Link kopiert', 'Instagram unterstützt kein direktes Teilen per Web. Der Link wurde kopiert.');
      return;
    }

    if (channel === 'email') {
      window.location.href = `mailto:?subject=${encodeURIComponent('Umzugsnetz empfehlen')}&body=${encodeURIComponent(`${shareText}\n\n${shareUrl}`)}`;
      return;
    }

    const targetUrl = channel === 'facebook'

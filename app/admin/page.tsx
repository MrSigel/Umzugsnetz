                ...
                <div className="max-w-lg space-y-6 py-12">
                  ...
                  <div>
                    <h1 className="text-4xl font-black tracking-tight sm:text-5xl lg:text-6xl">
                      Zentrale Steuerung für Kundenanfragen, Partner und Freigaben.
                    </h1>
                    <p className="mt-4 max-w-md text-base leading-relaxed text-white/72 sm:text-lg">
                      Ein ruhiges, hochwertiges Login mit direkter Umzugsnetz-Optik. Anmeldung und Admin-Registrierung bleiben auf einer einzigen, klaren Oberfläche.
                    </p>
                  </div>
                </div>
              ...
              <div className="mx-auto w-full max-w-md">
                ...
                <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-xl shadow-slate-200/50 sm:p-7">
                  <div className="mb-6 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1">
                    <button type="button" onClick={() => { setActiveTab('login'); resetMessages(); }} className={`rounded-xl px-4 py-3 text-sm font-bold transition-all ${activeTab === 'login' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Anmelden</button>
                    <button type="button" onClick={() => { setActiveTab('register'); resetMessages(); }} className={`rounded-xl px-4 py-3 text-sm font-bold transition-all ${activeTab === 'register' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Registrieren</button>
                  </div>
                  ...
                  {activeTab === 'login' ? (
                    <form onSubmit={handleLogin} className="space-y-6">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-12 py-4 text-slate-900 transition-all focus:border-brand-blue focus:outline-none focus:ring-2 focus:ring-brand-blue/15" placeholder="admin" required />
                        </div>
                      </div>
                      <div>
                      ...
                      <button type="submit" disabled={isLoading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-brand-blue py-4 font-bold text-white shadow-lg shadow-brand-blue/20 transition-all hover:bg-brand-blue-hover disabled:opacity-70">
                    ...
                    <form onSubmit={handleRegister} className="space-y-6">
                      <div>
                        ...
                        <div className="relative">
                          <User className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                          <input type="text" value={registerUsername} onChange={(e) => setRegisterUsername(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-12 py-4 text-slate-900 transition-all focus:border-brand-blue focus:outline-none focus:ring-2 focus:ring-brand-blue/15" placeholder="admin" required />
                        </div>
                      </div>
                      <div>
                        <label className="mb-2 block text-sm font-semibold text-slate-700">Passwort</label>
                        <div className="relative">
                          <Lock className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                          <input type="password" value={registerPassword} onChange={(e) => setRegisterPassword(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-12 py-4 text-slate-900 transition-all focus:border-brand-blue focus:outline-none focus:ring-2 focus:ring-brand-blue/15" placeholder="Mindestens 8 Zeichen" required />
                        </div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-12 py-4 text-slate-900 transition-all focus:border-brand-blue focus:outline-none focus:ring-2 focus:ring-brand-blue/15" placeholder="Einladungscode eingeben" required />
                        </div>
                      </div>
                      {error && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="rounded-2xl border border-red-100 bg-red-50 p-4 text-sm font-medium text-red-600">{error}</motion.div>}
                      {success && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-medium text-emerald-700">{success}</motion.div>}
                      <button type="submit" disabled={isLoading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-brand-green py-4 font-bold text-white shadow-lg shadow-emerald-500/20 transition-all hover:bg-brand-green-hover disabled:opacity-70">

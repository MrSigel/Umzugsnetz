...
type PortalTab = 'dashboard' | 'leads' | 'orders' | 'finance' | 'partners' | 'team' | 'settings';
...
export default function AdminPage() {
  ...
  if (loading) {
    return <main className="min-h-screen bg-slate-50 p-6 text-slate-900">Lädt...</main>;
  }
    ...
    <main className="min-h-screen bg-[#f5f8fb] text-slate-900">
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        <aside className="border-r border-slate-200 bg-white p-5">
          <div className="mb-8 rounded-[1.5rem] bg-slate-950 p-5 text-white">
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-blue">
              <Shield className="h-6 w-6" />
            </div>
            <p className="text-xl font-black">Lead Portal</p>
            <p className="mt-1 text-sm font-bold text-white/60">{isAdmin ? 'Admin Full Access' : 'Mitarbeiter'}</p>
          </div>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-black transition-all ${
                  activeTab === tab.id ? 'bg-brand-blue text-white shadow-lg shadow-brand-blue/20' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'
                }`}
              >
        ...
        <section className="p-4 sm:p-6 lg:p-8">
          <header className="mb-8 flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.22em] text-brand-blue">Admin Panel</p>
              <h1 className="mt-2 text-3xl font-black text-slate-950">Leads, Kunden und Systemsteuerung</h1>
            </div>
            <div className="flex flex-wrap gap-3">
              <select value={period} onChange={(event) => setPeriod(event.target.value)} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold outline-none focus:border-brand-blue">
                <option>Alle</option>
                <option>7 Tage</option>
                <option>30 Tage</option>
              </select>
              <select value={city} onChange={(event) => setCity(event.target.value)} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold outline-none focus:border-brand-blue">
                <option>Alle</option>
          ...
          {activeTab === 'leads' || activeTab === 'orders' ? (
            <div className="grid gap-5 xl:grid-cols-[1fr_420px]">
              <div className="space-y-5">
                <div className="flex flex-col gap-3 rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center">
          ...
          {activeTab === 'team' && isAdmin ? <TeamPanel members={data.team} onStatus={updateTeamStatus} /> : null}
...
function LeadList({ leads, onSelect, onStatusChange, orderMode = false }: { leads: Lead[]; onSelect: (lead: Lead) => void; onStatusChange: (lead: Lead, updates: Record<string, unknown>) => void; orderMode?: boolean }) {
  return (
    <div className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
      {leads.map((lead) => (
        <div key={lead.id} className="grid gap-4 border-b border-slate-100 p-4 last:border-b-0 lg:grid-cols-[1fr_auto]">
...
function LeadDetail({ lead, noteDraft, setNoteDraft, onSaveNote }: { lead: Lead | null; noteDraft: string; setNoteDraft: (value: string) => void; onSaveNote: (lead: Lead) => void }) {
  if (!lead) {
    return <div className="rounded-[1.75rem] border border-dashed border-slate-300 bg-white p-8 text-center font-bold text-slate-400">Lead auswählen</div>;
  }
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      {partners.map((partner) => (
        <div key={partner.id} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
...
function TeamPanel({ members, onStatus }: { members: TeamMember[]; onStatus: (member: TeamMember, status: string) => void }) {
  return (
    <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
      {members.map((member) => (
  ...
  return (
    <div className="grid gap-4">
      {settings.map((setting) => (
        <div key={setting.id} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">

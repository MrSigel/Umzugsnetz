...
export default function MarketplacePage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      setOrders((purchasedOrders || []).sort((leftOrder, rightOrder) => {
        return purchasedIds.indexOf(leftOrder.id) - purchasedIds.indexOf(rightOrder.id);
      }));
    } catch (error: any) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10 font-sans">
            ...
            <div className="bg-slate-50 rounded-2xl p-5 mb-6 space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-bold text-slate-800">
                  {confirmOrder.von_city}{confirmOrder.nach_city ? ` → ${confirmOrder.nach_city}` : ''}
                </span>
              </div>
              <div className="flex justify-between">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 transition-all">
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
        <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full lg:w-auto">
          <button
            ...
            className={`flex-1 lg:flex-none px-8 py-3 rounded-xl text-xs font-bold transition-all ${activeTab === 'available' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Offene Anfragen ({tabCounts.available})
          </button>
          <button
            onClick={() => setActiveTab('purchased')}
            className={`flex-1 lg:flex-none px-8 py-3 rounded-xl text-xs font-bold transition-all ${activeTab === 'purchased' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
        ...
        {loading ? (
                    ...
                    <div className="grid grid-cols-2 gap-6 py-6 border-y border-slate-50">
                      <div className="space-y-1">
                        <p className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          <Calendar className="w-3.5 h-3.5" /> Umzugsdatum
                        </p>
                        <p className="text-sm font-bold text-slate-700">
                          {order.move_date ? new Date(order.move_date).toLocaleDateString('de-DE', { day: '2-digit', month: 'long', year: 'numeric' }) : 'Flexibel'}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          <Package className="w-3.5 h-3.5" /> Umfang
                        </p>
                        <p className="text-sm font-bold text-slate-700">
                          {order.rooms_info || '—'} {order.size_info ? `/ ${order.size_info}` : ''}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          <ShieldCheck className="w-3.5 h-3.5" /> Kunden-Schätzpreis
                        </p>
                    ...
                    {activeTab === 'available' ? (
                      ...
                      >
                        {isPurchasing ? (
                          ...
                          <>Details & Kauf <ShoppingBag className="w-4 h-4" /></>
                        )}
                      </button>
                    ) : (
                      <span className="flex items-center gap-2 text-emerald-600 text-xs font-bold">
                        <CheckCircle2 className="w-4 h-4" /> Gekauft
                      </span>
                    )}
                  </div>
                </motion.div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-20 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-2xl font-black text-slate-800 mb-4 tracking-tight">
              {activeTab === 'available' ? 'Aktuell keine passenden Kundenanfragen' : 'Noch keine Leads gekauft'}
            </h3>
            ...
            <button onClick={fetchData} className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:bg-slate-800 shadow-xl shadow-black/10 transition-all flex items-center gap-2">
              Aktualisieren <RefreshCw className="w-4 h-4" />
            </button>
          </div>
        )}
      </div>
